...
public class PerformanceControllerTest2 {
    ...
    @Test
    void testUserAccessPerformanceAdd() throws Exception {
        // Setup mock Festival
        Festival festival = new Festival();
        festival.setFestivalId(1L);
        festival.setGenre(new Genre());
        festival.setRegion(new Region());
        Mockito.when(festivalRepository.findById(1L)).thenReturn(Optional.of(festival));

        // Save the Festival entity
        Mockito.when(festivalRepository.save(festival)).thenReturn(festival);

        // Mock PerformanceService to handle Performance entity
        Performance performance = new Performance();
        performance.setFestival(festival);
        Mockito.doNothing().when(performanceService).setupAddPerformanceFormModel(1L, performance, null);

        // Perform the test
        mockMvc.perform(get("/performance/add?festivalId=1")).andExpect(status().isOk());
    }

    @WithMockUser(username = "user", roles = {"USER"})
    @Test
    void testUserAccessPerformanceRemove() throws Exception {
        mockMvc.perform(get("/performance/remove?festivalId=1")).andExpect(status().isOk());
    }
